...
class KnowledgeBase:
        ...
        try:
            ...
            for i, chunk in enumerate(chunks):
                ...
                self.chunk_metadata.append(
                    {"source": source_name, "index": i, "type": os.path.splitext(file_path)[1]}
                )
            ...
            if embeddings.shape[1] != self.embedding_dim:
                raise ValueError(
                    f"Embedding dimension mismatch: {embeddings.shape[1]} != {self.embedding_dim}"
                )
    ...
    def retrieve_relevant(self, query, doc_processor, top_k=3):
        ...
        query_embedding = (
            doc_processor.embedding_model.encode([query])[0].reshape(1, -1).astype("float32")
        )
    ...
    def save(self):
        ...
        try:
            ...
            with open(data_path, "wb") as f:
                pickle.dump({"chunks": self.chunks, "chunk_metadata": self.chunk_metadata}, f)
